...
class EPRDataCollector:
    ...
    def __init__(self, 
        ...
        self.trig_count = triggerCount
        self.trig_source = triggerSource
        self.high_V = highV
        self.low_V = lowV
        self.high_time = highT
        self.low_time = lowT
        self.how_long_collect = collectionLeng
        self.how_many_cycles = collectionTimeToNumCycles(self.how_long_collect, self.trig_count, self.low_time, self.high_time)
        self.folder = rawdatafolder
        self.rawfilepath = self.folder + '/' + util.dtStringForFilename() + '.csv'
        ...
        self.daq_task = None
        self.gpib_adaptor = None
        self.dmm = None

    def initializeSaveFile(self):
        ...
        message = 'Output file created at '+ fp
        return message, file
    ...
    def initializeFrequencyCounter(self):
        ...
        freq_counter = self.resource_manager.open_resource(self.keysight_addr)
        ...
        freq_counter.write(getTrigSourceCmd(self.trig_source))
        freq_counter.write('TRIG:SLOP POS')
        freq_counter.write(getTrigCountCmd(self.trig_count))

    def initializeDAQ(self):
        # DAQ Setup and task initialization
        self.daq_task = nidaqmx.Task()
        self.daq_task.ao_channels.add_ao_voltage_chan(self.daq_path)
        self.daq_task.start()
        self.daq_task.write(0.0)#make sure we are starting at 0V

    def initializeDMM(self):
        
        self.gpib_adaptor = PrologixAdapter(self.dmm_addr, self.gpib_channel_no) #create prologix adapter and connect to GPIB w/ address 1
        self.dmm = Keithley2000(self.gpib_adaptor) #create the instrument using the adapter

        # Keithley data collection set up
        ## reset everything and clear the event queues
        self.dmm.reset()
        ## need to set trigger type to external
        self.dmm.write(trig_source_cmd)
        ## set trigger count to the desired number of datapoints per collection cycle
        self.dmm.write(trig_count_cmd)
